...
4. Read `public/_mockups/mockup-descriptions.json` → `chosen.file` → read the HTML mockup.
...
Write a `foundation-brief.md` using the same template as Path A, but structured as a diff:
