...
> Read this file after the foundation brief is approved. Execute changes following the brief precisely — it is your spec. The brief may include additional primitives beyond the 7 Required — handle them the same way.
...
**Before making any code changes**, immediately update `.github/flow-generator/FE/specs/foundation-manifest.json`:

1. Set `status` to `"in-progress"`
2. Set `scenario` from the brief's context (e.g., `"A"`, `"B"`, `"C"`)
3. Set `strategy` from the scenario (`"fresh-mockup"`, `"update-from-mockup"`, `"standalone-interview"`)
4. Set `source_mockup` from the brief's frontmatter (e.g., `"mockup-r2-v3.html"`, or `null` for scenario C)
5. Set `last_completed_step` to `"started"`
6. Reset all `quality_gates` to `"not_run"`
7. Clear `files_modified` to `[]`, `completed_at` to `null`
...
- `skills/styling/token-customization.patterns.md` — §Step 2 (config structure), §Step 3 (CSS custom props), §Color Scale Structure
- `skills/ui-primitives/customization.patterns.md` — §Font Registration (layout.tsx imports, CSS variables, tailwind fontFamily)
...
After gates G0–G2 pass, immediately update `foundation-manifest.json`:
...
For variant customization patterns (Typography variant discovery, Button/Badge cva updates, standard variant coverage), follow `skills/ui-primitives/customization.patterns.md`.
...
After gates G3–G4 pass, immediately update `foundation-manifest.json`:
...
After gate G5 passes, immediately update `foundation-manifest.json`:
...
After gate G6 passes, immediately update `foundation-manifest.json`:
...
After all gates pass (or are explicitly skipped by user), update `foundation-manifest.json` to its final state:
