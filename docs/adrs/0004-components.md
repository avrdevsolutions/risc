...
| --------------- | ---------------------- | ---------------------------------------------- | ----------------------------- |
| **1: UI**       | `components/ui/`       | Reusable primitives (buttons, cards, inputs)   | Nothing from `components/`    |
| **2: Features** | `components/features/` | Feature-specific (user profile, checkout form) | `ui/`, other `features/`      |
| **3: Layout**   | `components/layout/`   | App-wide chrome (header, footer, sidebar, nav) | `ui/`, `features/`            |
| **Page**        | `app/*/_components/`   | Page-private (not reusable)                    | `ui/`, `features/`, `layout/` |

**Import boundary rule:** Higher tiers may import lower tiers. Lower tiers MUST NOT import higher tiers.
...
ui/button/
  Button.tsx
  Button.stories.tsx       # When Storybook opted in (ADR-0002)
  index.ts
```
...
When a feature component is composed of multiple sub-components that together form a single unit (e.g., a card with a header, body, image, and actions), all parts MUST live in their own folder under the feature domain. The folder contains every piece of the composition and exports a clean public API via barrel.
...
| Use a `components/` sub-folder only when the composition has hooks, utils, types, or >5 sub-components | **SHOULD** |

```tsx
// ✅ features/landing/pricing-card/index.ts — only main component is public
...
| Import `features/` from `ui/`                                | Lower tier must not import higher                  |
...
- ~140 line splitting guideline keeps files reviewable and diffable.
