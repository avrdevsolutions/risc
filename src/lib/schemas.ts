...
export const ProiectSchema = z
  .object({
    denumireProiect: z.string().min(3, 'Denumirea proiectului este obligatorie (min. 3 caractere)'),
    ...
    antreprenor: z.string().min(2, 'Antreprenorul este obligatoriu'),
    cuiAntreprenor: z.string().optional(),
    subantreprenor: z.string().optional(),
    fazaLucrarii: z.string().min(1, 'Selectați faza lucrării'),
    fazaLucrariiCustom: z.string().optional(),
    descriereObiectiv: z.string().optional(),
  })
  .refine(
    (data) =>
      data.fazaLucrarii !== 'Altă fază' ||
      (data.fazaLucrariiCustom && data.fazaLucrariiCustom.length >= 3),
    { message: 'Specificați faza lucrării', path: ['fazaLucrariiCustom'] },
  )
...
export const RiscSchema = z
  .object({
    activitate: z.string().min(1, 'Selectați activitatea'),
    activitateCustom: z.string().optional(),
    pericol: z.string().min(1, 'Selectați pericolul'),
    pericolCustom: z.string().optional(),
    descrierePericol: z.string().optional(),
    persoaneExpuse: z.array(z.string()).min(1, 'Selectați cel puțin o categorie'),
    ...
    masuriExistente: z.array(z.string()).min(1, 'Selectați cel puțin o măsură'),
...
export const AprobareSchema = z.object({
  sefSantier: z.string().min(3, 'Numele șefului de șantier este obligatoriu'),
  functieSefSantier: z.string().optional(),
  responsabilSSM: z.string().min(3, 'Numele responsabilului SSM este obligatoriu'),
